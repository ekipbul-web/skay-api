...
foreach ($dnsTypes as $type) {
    if ($type === 'A') $records = @dns_get_record($domain, DNS_A);
    elseif ($type === 'MX') $records = @dns_get_record($domain, DNS_MX);
    elseif ($type === 'NS') $records = @dns_get_record($domain, DNS_NS);
    else $records = [];
    
    if ($records) {
        $values = [];
        foreach ($records as $rec) {
            if (isset($rec['target'])) $values[] = $rec['target'];
            elseif (isset($rec['ip'])) $values[] = $rec['ip'];
        }
        $result['dns'][$type] = array_slice($values, 0, 3);
    }
}
...
if ($crt) {
    ...
    $subs = [];
    foreach ($crtData as $entry) {
        if (isset($entry['name_value'])) $subs[] = $entry['name_value'];
    }
    $result['subdomains'] = array_slice(array_unique($subs), 0, 10);
}
