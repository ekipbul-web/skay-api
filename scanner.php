...
if ($ip && $ip != $domain) {
    ...
    if ($geo) {
        ...
        if (is_array($geoData)) {
            $result['country'] = isset($geoData['country']) ? $geoData['country'] : null;
            $result['city'] = isset($geoData['city']) ? $geoData['city'] : null;
            $result['isp'] = isset($geoData['isp']) ? $geoData['isp'] : null;
        }
    }
}
...
if ($ip) {
    ...
    if ($whois) {
        $whoisKeys = ['Registrar', 'Creation Date', 'Registry Expiry Date'];
        foreach ($whoisKeys as $key) {
