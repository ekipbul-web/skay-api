...
if (!$sehir) {
    echo json_encode([
        'status' => 'error',
        'message' => 'sehir parametresi gerekli. Ornek: ?sehir=istanbul/kadikoy'
    ], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
    exit;
}

// İl/ilçe ayır
if (strpos($sehir, '/') !== false) {
    list($il, $ilce) = explode('/', $sehir, 2);
    $il = trim($il);
    $ilce = trim($ilce);
} else {
    $il = trim($sehir);
...
$query = $ilce ? $ilce : $il;
$url = "http://api.aladhan.com/v1/timingsByCity?city=" . urlencode($query) . "&country=Turkey&method=13";
...
// İlçe çalışmazsa il ile dene
if ($data['code'] != 200 && $ilce) {
...
if ($ilce) {
    $osm_query = urlencode("$ilce, $il, Turkey");
} else {
    $osm_query = urlencode("$il, Turkey");
}

$osm_url = "https://nominatim.openstreetmap.org/search?q=$osm_query&format=json&limit=1";
